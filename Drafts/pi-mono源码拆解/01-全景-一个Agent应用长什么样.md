...
有一个细节值得留意：pi-agent-core 的 `Agent` 类接收一个 `convertToLlm` 函数，把 `AgentMessage[]` 转成 LLM 能理解的 `Message[]`。这意味着 Agent 内部的消息格式和 LLM 的消息格式是解耦的——你可以在对话历史里塞一条"用户切换了分支"的通知，UI 渲染它，但 `convertToLlm` 把它过滤掉，LLM 根本看不到。这个设计下一篇会展开。
