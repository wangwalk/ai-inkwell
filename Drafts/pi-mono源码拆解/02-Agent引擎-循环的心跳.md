...
一次典型的执行路径：用户说"把错误处理加上" → LLM 说要先读文件（tool_use）→ 读文件 → LLM 说要改代码（tool_use）→ 改代码 → LLM 说要跑测试（tool_use）→ 跑测试 → LLM 说搞定了（stop）→ 内层循环退出 → 没有 follow-up → 外层循环也退出。
...
| stopReason | 含义 | 循环行为 |
|-----------|------|---------|
| stop | LLM 认为任务完成 | 退出内层循环，检查 follow-up |
| toolUse | LLM 要调工具（各 provider 统一映射为此值） | 执行工具，结果塞回，继续循环 |
| error | API 报错 | 立即退出整个循环 |
| aborted | 用户主动取消 | 立即退出整个循环 |

注意，没有一个显式的状态机或 switch-case 来路由这四种状态。它完全靠循环条件自然处理：`hasMoreToolCalls` 为 false 时内层循环退出（对应 stop），为 true 时继续（对应 toolUse），error/aborted 直接 return。

这是一种隐式状态机——状态转换嵌在控制流里，而不是用一个 `state` 变量显式管理。代码更短，但你得通读整个循环才能理解所有可能的路径。

## 两层消息抽象：AgentMessage vs Message

上一篇提到，Agent 内部的消息格式和 LLM 的消息格式是解耦的。为什么？答案就在这个循环里。
...
```typescript
// packages/agent/src/agent-loop.ts — executeToolCalls 内部
...
```typescript
// packages/agent/src/agent.ts — _runLoop 内部
...
如果哪天要支持并行工具执行，最先碰到的问题不是性能，而是 steering 的语义：三个工具同时在跑，用户发了一条 steering 消息，是等全部完成再检查，还是任何一个完成就检查？前者简单但延迟高，后者响应快但要处理半完成状态。这个问题没有标准答案，取决于你的 Agent 面对的是什么场景。
